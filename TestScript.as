...
//	GetProxyVec3dArray is no longer registered with AngelScript.  It converts the manager's
//	staging buffer, which the array-returning API functions fill just before calling it, so on
//	its own it handed back whatever the previous call happened to leave there.
