...
SuperConeScreen::SuperConeScreen( ScreenMgr* mgr ) : GeomScreen( mgr, 400, 800, "SuperCone" )
{
    ...
    m_DesignLayout.AddSlider( m_SuperConeDistanceSlider, "Distance", 10, "%7.3f" );
...
//==== Show SuperCone Screen ====//
void SuperConeScreen::Show()
{
    if ( Update() )
    {
        GeomScreen::Show();
...
//==== Update SuperCone Screen ====//
bool SuperConeScreen::Update()
{
    assert( m_ScreenMgr );

    Geom* geom_ptr = m_ScreenMgr->GetCurrGeom();
    if ( !geom_ptr || geom_ptr->GetType().m_Type != SUPER_CONE_GEOM_TYPE )
    {
        Hide();
        return false;
    }
    ...
    //==== Update SuperCone Specific Parms ====//
    SuperConeGeom* bor_ptr = dynamic_cast< SuperConeGeom* >( geom_ptr );
    assert( bor_ptr );

    m_SuperConeDistanceSlider.Update( bor_ptr->m_Distance.GetID() );
...
void SuperConeScreen::GuiDeviceCallBack( GuiDevice* gui_device )
{
    //==== Find SuperCone Ptr ====//
    Geom *geom_ptr = m_ScreenMgr->GetCurrGeom();
    if ( !geom_ptr || geom_ptr->GetType().m_Type != SUPER_CONE_GEOM_TYPE )
    ...
    SuperConeGeom *scone_ptr = dynamic_cast< SuperConeGeom * >( geom_ptr );

    if (gui_device == &m_XSecTypeChoice)
    {
        int t = m_XSecTypeChoice.GetVal();
        scone_ptr->SetXSecCurveType( t );
    ...
    {
        EditCurveXSec* edit_xsec = scone_ptr->ConvertToEdit();
    ...
    {
        XSecCurve* xsc = scone_ptr->GetXSecCurve();
        if ( xsc )
        {
            ...
            ( gui_device == &m_OneSixSeriesFitCSTButton ) )
    {
        ...
        if ( xsc )
        {
            ...
            if ( af_xs )
            {
                ...
                scone_ptr->SetXSecCurveType( vsp::XS_CST_AIRFOIL );

                XSecCurve* newxsc = scone_ptr->GetXSecCurve();
                if ( newxsc )
                {
...
//==== Non Menu Callbacks ====//
void SuperConeScreen::CallBack( Fl_Widget *w )
{
    m_XsecAttributeEditor.DeviceCB( w );
    GeomScreen::CallBack( w );
}

void SuperConeScreen::DisplayGroup( GroupLayout* group )
{
...
void SuperConeScreen::RebuildCSTGroup( CSTAirfoil* cst_xs)
{
    if ( !cst_xs )
    {
        return;
