...
// ParmLinkScreenProg.cpp: The programmatic implementation of the Parameter Link GUI
//
//////////////////////////////////////////////////////////////////////
...
ParmLinkScreenProg::ParmLinkScreenProg( ScreenMgr* mgr ) : BasicScreen( mgr, 600, 595, "Parm Link  ( B = A*scale + offset)" )
{
    ...
    m_GenLayout.AddSlider( m_ScaleSlider, "Scale", 1, " %7.5f" );
    m_GenLayout.ForceNewLine();
    m_GenLayout.SetFitWidthFlag( true );
    m_GenLayout.SetSameLineFlag( false );
...
ParmLinkScreenProg::~ParmLinkScreenProg()
{
}

void ParmLinkScreenProg::Show()
{
    BasicScreen::Show();
...
bool ParmLinkScreenProg::Update()
{
    int i;
    char str[256];
...
void ParmLinkScreenProg::CallBack( Fl_Widget* w )
{
    assert( m_ScreenMgr );
...
void ParmLinkScreenProg::GuiDeviceCallBack( GuiDevice* device )
{
    assert( m_ScreenMgr );
    Link* currLink = LinkMgr.GetCurrLink();
