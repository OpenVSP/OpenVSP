...
*/)";
    ...
    doc_struct.comment = R"(
/*!
    If no MeshGeom exists in the model, this creates a MeshGeom of mixed thick and thin geometry and exports that file from OpenVSP.
    Only vspgeom supports this at this time, for all other types, this behaves the same as ExportFile (ignoring the thin_set parameter).  If a mesh is generated for a particular export,
    the ID of the MeshGeom will be returned. If no mesh is generated an empty string will be returned.
    \code{.cpp}
    string pid = AddGeom( "POD" );
    ...
    SetParmVal( wid, "X_Rel_Location", "XForm", 2.0 );

    Update();

    string mesh_id = ThickThinExportFile( "thickthin.vspgeom", 3, 4, EXPORT_VSPGEOM );

    DeleteGeom( mesh_id ); // Delete the mesh generated by the export
    \endcode
    \sa EXPORT_TYPE
    \param [in] file_name Export file name
    \param [in] thick_set Set index to export as thick geometry (i.e. SET_ALL)
    \param [in] thin_set Set index to export as thin geometry (i.e. SET_ALL)
    \param [in] file_type File type enum (i.e. EXPORT_VSPGEOM)
    \return Mesh Geom ID if the export generates a mesh
*/)";
    r = se->RegisterGlobalFunction( "string ThickThinExportFile( const string & in file_name, int thick_set, int thin_set, int file_type )", asFUNCTION( vsp::ThickThinExportFile ), asCALL_CDECL, doc_struct );
    assert( r >= 0 );
