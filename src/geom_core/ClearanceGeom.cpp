...
ClearanceGeom::ClearanceGeom( Vehicle* vehicle_ptr ) : Geom( vehicle_ptr )
{
    ...
    m_Offset.SetDescript( "Internal Offset Distance to Clearance Surface" );
...
//==== Approximate Center ====//
vec3d ClearanceGeom::ComputeCenter( curve_type & crv )
{
    vec3d sum(0,0,0);
...
//==== Translate Curve ====//
void ClearanceGeom::TranslateCurve( curve_type & crv, const vec3d & offset )
{
...
void ClearanceGeom::OffsetCurve( curve_type & crv, double offset )
{
...
//==== Force Reskin ====//
void ClearanceGeom::ReSkin( VspSurf & surf, vector< rib_data_type > & rib_vec )
{
    //==== Reskin Surface ====//
...
bool ClearanceGeom::CheckIfRibIsPoint( rib_data_type & rib )
{
...
void ClearanceGeom::CenterRibCurves( VspSurf & surf, const VspSurf & ref_surf, double offset )
{
...
void ClearanceGeom::AdjustShape( VspSurf & surf, const VspSurf & ref_surf, double offset )
{
    vector< rib_data_type > rib_vec;
    surf.GetSkinRibVec( rib_vec );
...
void ClearanceGeom::OffsetEndRibs( VspSurf & surf, double offset )
{
    vector< rib_data_type > rib_vec;
    surf.GetSkinRibVec( rib_vec );
...
double ClearanceGeom::FindEndUOffsetCurve( VspSurf & surf, double offset, bool start_flag, curve_type & crv )
{
    ClearanceSpine cs;
    cs.Build( surf );
    double u = 0.0;
...
//==== End Points Of Clearance Surf Are Moved Offset Dist But Still May Be Too Close To Ref Surf  ===//
//==== Adjust Location Until Closest Dist Matches Offset ====//
double ClearanceGeom::AdjustForSurfaceDist( const VspSurf & surf, const ClearanceSpine & spine, double u, double offset, bool backward_flag )
{
    double su, sw;
    double start_u = u;
    double stop_u = u;
...
void ClearanceGeom::SetWingTrimParms(  VspSurf & surf )
{
...
void ClearanceGeom::UpdateParms( VspSurf & surf )
{
    Geom* parent_geom = m_Vehicle->FindGeom( m_ParentID );
    if ( !parent_geom )
    {
...
void ClearanceGeom::TrimU( VspSurf & surf )
{
    if ( !m_UTrimFlag() )
    {
        return;
...
void ClearanceGeom::TrimV( VspSurf & surf )
{
...
void ClearanceGeom::FindDists( const VspSurf & surf, piecewise_curve_type & curve, double u0, vector< double > & dist_vec )
{
...
double ClearanceGeom::ComputeAvgOffset( VspSurf & surf, const VspSurf & ref_surf, double u )
{
    VspCurve crv;
...
double ClearanceGeom::ComputeMaxOffsetError( VspSurf & surf, VspSurf &  ref_surf, double offset, int num_u, int num_w )
{
    VspCurve crv;
    double max_err = 0.0;
    int sum_cnt = 0;
...
void ClearanceSpine::Build( const VspSurf & surf )
{
    //===== Clear Data ====//
    m_UVec.clear();
    m_CenterVec.clear();
...
double ClearanceSpine::FindUGivenDist( double d ) const
{
    double ret_u = 0.0;
...
vec3d ClearanceSpine::FindCenterGivenU( double u ) const
{
    vec3d center;
...
double ClearanceSpine::GetSpineLength() const
{
    double length = 0;
...
double ClearanceSpine::FindUGivenLengthAlongSpine( double length ) const
{
    double u = 0;
