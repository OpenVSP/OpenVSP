...
xmlNodePtr VSPAEROMgrSingleton::DecodeXml( xmlNodePtr & node )
{
    ...
    if ( VSPAEROsetnode )
    {
        ...
        m_RefGeomID = IDMgr.RemapRefID( XmlUtil::FindString( VSPAEROsetnode, "RefGeomID", m_RefGeomID ) );
        m_ModeID = IDMgr.RemapRefID( XmlUtil::FindString( VSPAEROsetnode, "ModeID", m_ModeID ) );
        m_CGModeID = IDMgr.RemapRefID( XmlUtil::FindString( VSPAEROsetnode, "CGModeID", m_CGModeID ) );
...
xmlNodePtr RotorDisk::DecodeXml( xmlNodePtr & node )
{
    ...
    if ( node )
    {
        ...
        m_ParentGeomId = IDMgr.RemapRefID( XmlUtil::FindString( node, "ParentID", defstr ) );
...
xmlNodePtr ControlSurfaceGroup::DecodeXml( xmlNodePtr & node )
{
    ...
    if ( node )
    {
        m_ParentGeomBaseID = IDMgr.RemapRefID( XmlUtil::FindString( node, "ParentGeomBase", ParentGeomID ) );
...
xmlNodePtr UnsteadyGroup::DecodeXml( xmlNodePtr& node )
{
    if ( node )
    {
        ...
        for ( size_t i = 0; i < nComponents; ++i )
        {
            ...
            string compID = IDMgr.RemapRefID( XmlUtil::FindString( csnode, "CompID", "" ) );
