                                           ...
                                           const vector <string> & Flow_vec, bool Symm, bool useMode, const string &modeID )
{
    ...
    res->Add( new NameValData( "Mesh_GeomID", m_lastmeshgeomID, "GeomID of MeshGeom created." ) );
...
{
    res->Add( new NameValData( "CDWave", m_CDWave, "Wave drag coefficient." ) );

    res->Add( new NameValData( "Length", m_Length, "Equivalent body length." ) );
    res->Add( new NameValData( "Volume", m_Volume, "Equivalent body volume." ) );
    res->Add( new NameValData( "MaxArea", m_MaxArea, "Equivalent body max area." ) );
}
...
void WaveDragSingleton::PushSliceResults( Results* res )
{
    ...
    res->Add( new NameValData( "Ambiguous_Subsurf_Flag", m_AmbigSubSurf, "Ambiguous subsurface flag." ) );

    res->Add( new NameValData( "Num_Slices", m_NSlice, "Number of slices." ) );
    ...
    res->Add( new NameValData( "Theta", theta_deg, "Theta angle around cone of influence." ) );
    res->Add( new NameValData( "Num_Cone_Sections", m_NTheta, "Number of thetas." ) );

    res->Add( new NameValData( "Inlet_Area", m_InletArea, "Propulsive inlet area." ) );
    res->Add( new NameValData( "Exit_Area", m_ExitArea, "Propulsive exit area." ) );

    res->Add( new NameValData( "X_Norm", m_XNorm, "Slice normal vector." ) );
    res->Add( new NameValData( "Start_X", m_StartX, "Slice starting coordinate." ) );
    res->Add( new NameValData( "End_X", m_EndX, "Slice ending coordinate." ) );
}
