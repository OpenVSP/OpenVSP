...
VORTEX_SHEET& VORTEX_SHEET::operator+=(const VORTEX_SHEET &VortexSheet)
{
    ...
    Is2D_                     = VortexSheet.Is2D_;
    
    Level_                    = VortexSheet.Level_;
    
    SheetID_                  = VortexSheet.SheetID_;
...
void VORTEX_SHEET::SizeTrailingVortexList(int NumberOfTrailingVortices)
{
    ...
    if ( NumberOfTrailingVortices_ != 0 ) {
       ...
       for ( i = 0 ; i <= NumberOfTrailingVortices_ ; i++ ) {
           ...
           TrailingGamma_[i] = NULL;
           
           StartingGamma_[i] = NULL;
    ...
    for ( Level = 1 ; Level <= NumberOfLevels_ ; Level++ ) {
       ...
       TrailingVortexListForLevel_[Level] = NULL;
...
void VORTEX_SHEET::SetupPlanarVortexSheets(void)
{
    ...
    if ( Verbose_ ) printf("CoreSize_: %f \n",CoreSize_);
...
void VORTEX_SHEET::UpdateVortexStrengths(int UpdateType)
{
    ...
    for ( i = 1 ; i <= NumberOfTrailingVorticesForLevel_[Level] ; i++ ) {
     ...
     //  Level = 1;
     //
     //  for ( k = 1 ; k <= NumberOfTrailingVorticesForLevel_[Level] ; k++ ) {
     //     
     //     PRINTF("TrailingGammaListForLevel_[%d][%d][1]: %f \n",Level,k,TrailingGammaListForLevel_[Level][k][1]);
     //     
     //  }
...
/*##############################################################################
#                                                                              #
#              VORTEX_SHEET UpdateTrailingEdgeGeometryLocation                 #
#                                                                              #
##############################################################################*/

void VORTEX_SHEET::UpdateTrailingEdgeGeometryLocation(VSPAERO_DOUBLE *TVec, VSPAERO_DOUBLE *OVec, QUAT &Quat, QUAT &InvQuat, int *ComponentInThisGroup)
{
    ...
    for ( i = 1 ; i <= NumberOfTrailingVortices_ ; i++ ) {
       ...
       if ( ComponentInThisGroup[TrailingVortexList_[i]->ComponentID()] ) {
           ...
           TrailingVortexList_[i]->UpdateTrailingEdgeGeometryLocation(TVec, OVec, Quat, InvQuat);
...
int VORTEX_SHEET::FarAway(VSPAERO_DOUBLE xyz[3])
{
    ...
    if ( Distance_ >= 10.*FarAway_*Span_ ) return 1;
