...
#ifdef AUTODIFF
using namespace VSPAERO_ADJOINT;
#else
...
VSPAERO_DOUBLE CDtForCase[MAXRUNCASES];
...
void ComplexDiffTestSolve(void);
...
int main(int argc, char **argv)
{
    ...
    if ( RotorAnalysisRun_ > 0 ) VSP_VLM().RotorAnalysis() = RotorAnalysisRun_;
    ...
    else if ( DoComplexDiffTest > 0 ) {
       
       ComplexDiffTestSolve();
...
void PrintUsageHelp()
{
       ...
       PRINTF(" -hilift                            Process the geometry and write out a default high lift setup file. \n");
       ...
       PRINTF(" -qrotor                            Run a quasi-steady state rotor analysis, assumes .groups file is setup! \n");
...
void ParseInput(int argc, char *argv[])
{
    ...
    while ( i <= argc - 2 ) {
       ...
       else if ( strcmp(argv[i],"-qrotor") == 0 ) {
       ...
       else if ( strcmp(argv[i],"-hilift") == 0 ) {
        
          SetupHighLiftFile_ = 1;
       ...
       else if ( strcmp(argv[i],"-complextest") == 0 ) {
...
void RotorAnalysisSolve(void)
{
  ...
  // this is taken care of later...           VSP_VLM().RotationalRate_p() = BladeRPM_*2.*PI/60.;
...
void ComplexDiffTestSolve(void)
{
    ...
    FILE *ComplexStepFile;

    // Open the output file for the test derivatives
    
    SPRINTF(TestFileName,"%s.complex.gradient",FileName);

    if ( (ComplexStepFile = fopen(TestFileName,"w")) == NULL ) {

       PRINTF("Could not open the complex step output file! \n");
    ...
    FPRINTF(ComplexStepFile,"   Node            X          Y          Z            dFdX       dFdY       dFdZ \n");
...
void FiniteDiffTestSolve(void)
{
    ...
    int i, j, Case, NumCases;
    double Delta, Fo, *TempXYZ, dFdxyz[3];
    char TestFileName[2000];
    FILE *FiniteDiffFile;
  ...
  //    for ( i = 35 ; i <= 35 ; i+=1 ) {
        ...
        VSP_VLM().VSPGeom().Grid(0).NodeList(i).x() = TempXYZ[3*i-2];
        VSP_VLM().VSPGeom().Grid(0).NodeList(i).y() = TempXYZ[3*i-1];
        VSP_VLM().VSPGeom().Grid(0).NodeList(i).z() = TempXYZ[3*i  ];

        dFdxyz[0] = ( VSP_VLM().OptimizationFunction() - Fo )/Delta;
        ...
        dFdxyz[1] = ( VSP_VLM().OptimizationFunction() - Fo )/Delta;
        ...
        dFdxyz[2] = ( VSP_VLM().OptimizationFunction() - Fo )/Delta;
        ...
        FPRINTF(FiniteDiffFile,"%d %10.6e %10.6e %10.6e    %10.6e %10.6e %10.6e \n",
         i,
         VSP_VLM().VSPGeom().Grid(0).NodeList(i).x(),
         VSP_VLM().VSPGeom().Grid(0).NodeList(i).y(),
         VSP_VLM().VSPGeom().Grid(0).NodeList(i).z(),
