...
function TestVSPAeroCreateModel(~)
    ...
    assert_delta( vsp.SetParmValUpdate( wing_id, 'Tess_W', 'Shape', 20 ), calcTessWCheckVal( 20 ), TEST_TOL );
...
function TestVSPAeroControlSurfaceDeflection_test(~)
    ...
    execution_verbosity = vsp.IntVector(); execution_verbosity.push_back(vsp.NO_OUTPUT );
...
%     global TEST_TOL     % read in TEST_TOL from global workspace
    ...
    wakeNumIter = vsp.IntVector(); wakeNumIter.push_back(1);
...
%     global TEST_TOL     % read in TEST_TOL from global workspace
    ...
    wakeNumIter = vsp.IntVector(); wakeNumIter.push_back(1);
    vsp.SetIntAnalysisInput( analysis_name, 'WakeNumIter', wakeNumIter );
    wakeSkipUntilIter = vsp.IntVector(); wakeSkipUntilIter.push_back(2);
    vsp.SetIntAnalysisInput( analysis_name, 'WakeSkipUntilIter', wakeSkipUntilIter );
    stabilityCalcFlag = vsp.IntVector(); stabilityCalcFlag.push_back(1);
...
%     global m_vspfname_for_vspaerotests
   ...
   vsp.SetDriverGroup( wing_id, 1, vsp.AREA_WSECT_DRIVER, vsp.ROOTC_WSECT_DRIVER, vsp.TIPC_WSECT_DRIVER );
    ...
    wakeSkipUntilIter = vsp.IntVector(); wakeSkipUntilIter.push_back(2);
    vsp.SetIntAnalysisInput( analysis_name, 'WakeSkipUntilIter', vsp.IntVector(wakeSkipUntilIter) );
