...
extern int FindInterval( vector< double > & vals, double value )
{
	int dim = vals.size();
	if ( dim == 0 )
		return 0;
...
//==== Interpolate ===//
extern double Interpolate( vector< double > & arr, double value, int interval )
{
	int dim = arr.size();
	if (dim <= 0)				return( 0.0 );
