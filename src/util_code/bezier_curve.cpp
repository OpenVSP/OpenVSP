...
{	
  ...
  if ( sec_num < 0 ) return( pnts[0] );

  else if ( sec_num > (num_sections - 1) ) return( pnts[num_sections*3] );
...
void Bezier_curve::setTan( vec3d & tan, const vector< vec3d > & pVec, int ind, int flag, int forward, int close )
{
	if ( ind == 0 )
	{
		if ( close )	tan = pVec[1] - pVec[pVec.size()-2];
		else			tan = pVec[1] - pVec[0];
	...
	{
		if (close )		tan = pVec[1] - pVec[pVec.size()-2];
		else			tan = pVec[pVec.size()-1] - pVec[pVec.size()-2];
	...
	if ( flag == ZERO )				tan = vec3d(0,0,0);
	else if ( flag == SHARP )
	{
		if ( forward )	tan = pVec[ind+1] - pVec[ind];
		else			tan = pVec[ind] - pVec[ind-1];
	}		
	else if (  flag == ZERO_X )		tan.set_x(0.0);
	else if (  flag == ZERO_Y )		tan.set_y(0.0);
	else if (  flag == ZERO_Z )		tan.set_z(0.0);
	else if (  flag == ONLY_BACK )	tan = pVec[ind] - pVec[ind-1];
	else if (  flag == ONLY_FORWARD )tan = pVec[ind+1] - pVec[ind];
