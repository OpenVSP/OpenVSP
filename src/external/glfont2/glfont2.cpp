...
bool GLFont::Create (int tex)
{
	...
	// Pick out binary data length and cast unsigned char* to char*
	int len = sizeof(basic_font_uc);
	const char * basic_font_c = reinterpret_cast<const char*>(basic_font_uc);

	// Wash basic_font through string to ignore null characters.
	string s( basic_font_c, len );
