...
{
public:
	Node()									{ fixed = u_undef = w_undef = m_DeleteMeFlag = false; }
	Node( vec3d& p, vec2d& uw_in )			{ pnt = p; uw = uw_in; fixed = u_undef = w_undef = m_DeleteMeFlag = false; }
	virtual ~Node();
...
{
public:
	Edge()									{ n0 = n1 = NULL; t0 = t1 = NULL; ridge = border = debugFlag = m_DeleteMeFlag = false; splitFlag = true; }
	Edge( Node* node0, Node* node1 )		{ n0 = node0; n1 = node1; t0 = t1 = NULL; ridge = border = debugFlag = m_DeleteMeFlag = false; splitFlag = true; }
	virtual ~Edge()							{}
	...
	double length2()			{ return dist_squared( n0->pnt, n1->pnt ); }

	double ComputeLength()		{ m_Length = length(); return m_Length; }
